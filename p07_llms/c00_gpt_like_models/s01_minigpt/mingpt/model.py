...
Changes: A small modification in this file was made to the original code to make it compatible with PyTorch 2.9
and to allow for setups with multiple GPUs in the Block module.
"""
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        keys_hf = [
            k for k in keys_hf if not k.endswith(".attn.bias")
        ]  # ignore these, just a buffer
        transposed = [
            "attn.c_attn.weight",
            "attn.c_proj.weight",
        ...
        assert len(keys_hf) == len(
            sd_keys
        ), f"mismatched keys: {len(keys_hf)} != {len(sd_keys)}"
        for k in sd_keys:
